...
map<string, Uc> Data::getAllUcs() const
{
    return this->ucs;
}
...
Uc &Data::getUc(const string &ucCode)
{
    return this->ucs.at(ucCode);
}
...
map<int, Student> Data::getAllStudents() const
{
 ...
 * m is the number of UCs, and k is the number of classes of a given UC
...
/**
 * @brief Reads the classes.csv file. the classSchedules inside their corresponding class and uc are set to the read values.
 * @details Time complexity: O(n ( 2m + log(k) + log(v) ) where n is the number of lines in the file,
 * m is the length of the line, k is the number of UCs, and v is the number of classes of a given UC
...
void Data::readClassesFile(ifstream &file)
{
    ...
    while (getline(file, line)) // n
    {
        ...
        Class& currentClass = ucs.at(ucCode).getClass(classCode); // log(k) + log(v)
...
/**
 * @brief Reads the students_classes.csv file. the students is set to the read values.
 * @details Time complexity: O(n ( 3m + r + log(k) + log(v) + log(s) + 2log(u) + log(p) ) where n is the number of lines in the file,
 * m is the length of the line, r is ts the length of the name, k is the number of UCs, v is the number of classes of a given UC,
 * s is the number of students in a given class, u is the total number of students, and p is the number of UCs of a given student
 * @param file classes.csv file as ifstream
 */
...
void Data::readStudentsClassesFile(ifstream &file)
{
    ...
    while (getline(file, line)) // n
    {
        ...
        Class& currentClass = this->ucs.at(ucCode).getClass(classCode); // log(k) + log(v)
...
void Data::writeStudentsClassesFile(ofstream &file)
{
    ...
    for (const auto &[studentCode, student] : this->students) // n
    {
        ...
        {
            file << studentCode << "," << student.getStudentName() << "," << ucCode << "," << c.getClassCode() << "\n";
        }
    }
}